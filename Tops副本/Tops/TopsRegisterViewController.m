...
#define COMPANY_FIELD_TAG       6


@implementation TopsRegisterViewController
{
    NSData *m_JsonData;
...
- (void)viewDidLoad
{
    ...
    if (self.keyboardToolbar==nil) {
        ...
        self.companyTextField.inputAccessoryView=self.keyboardToolbar;  //控制虚拟键盘是否可用，和toolbar是否有效...否则:工具条不显示，键盘无法输入文本域
...
{
    if (self.nameTextField.text==nil ||[self.nameTextField.text length]==0||self.roleTextField.text == nil||[self.roleTextField.text length]==0 || self.phoneTextField.text == nil|| [self.phoneTextField.text length]==0
        ||self.companyTextField.text==nil||[self.companyTextField.text length]==0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"错误提示！" message:@"姓名，电话，职务，公司为必填项!" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alert show];
        [alert release];
    }else{
    NSPredicate *resultPredicate=[NSPredicate predicateWithFormat:@"SELF contains[cd] %@",self.companyTextField.text];
    NSArray *searchResults=[CompanysList filteredArrayUsingPredicate:resultPredicate];
    if ([searchResults count]==1) {
        //存档
        [self cundang];
        RegisterHttp *rh = [[RegisterHttp alloc] init];
        NSString * finished = [rh registerUser:[[ArchivingData alloc]initWithFirstname:self.nameTextField.text Lastname:self.lastNameTextField.text Role:self.roleTextField.text Phone:self.phoneTextField.text gender:self.gender Company:self.companyTextField.text]];
        [rh release];
        
        if (![finished isEqualToString:@"0"]) {
            
            NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
            //将服务器端注册Id存入NSUserDefaults中
            [userDefaults setObject:finished forKey:kMYID];
            //将注册状态设置为YES
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"注册失败！" message:@"请稍后再试!" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alert show];
            [alert release];
        }
        
    }else{
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"错误提示！" message:@"公司录入错误,请输入查询后,选择争取的公司名称!" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alert show];
        [alert release];
    }
...
//        NSLog(@"点击previous按钮：%d", previousTag);
        ...
        if (nextLabel) {
            [self resetLabelsColors];
            [nextLabel setTextColor:[TopsRegisterViewController labelSelectedColor]];
        }
...
{
    self.roleLabel.textColor = [TopsRegisterViewController labelNormalColor];
    self.genderLabel.textColor = [TopsRegisterViewController labelNormalColor];
    self.phoneLabel.textColor = [TopsRegisterViewController labelNormalColor];
    self.companyLabel.textColor=[TopsRegisterViewController labelNormalColor];
}
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (label) {
        [self resetLabelsColors];
        [label setTextColor:[TopsRegisterViewController labelSelectedColor]];
    }
}
